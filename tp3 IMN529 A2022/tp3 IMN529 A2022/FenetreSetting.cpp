					...
					"Setting")
{
	...
	int t = 15;
	int l = 15;
	AddControl(new cgFrame(l-5,t-5,230,220,"Options "));
...
//	CHKBOX_OPENGL =	AddControl(pOpenGL);
   ...
   pUtiliserRadiosite = new cgCheckBox(10+l, 90+t, 22, "Utiliser la radiosite");
   CHKBOX_RADIOSITE = AddControl(pUtiliserRadiosite);

	pMeridiens = new cgSpinner(10+l,t+120,2,3,99);
	SPIN_MERIDIENS = AddControl(pMeridiens);
	AddControl(new cgLabel(65+l,t+130," Meridien(s)"));
	pParalleles = new cgSpinner(10+l,t+145,2,1,99);
	SPIN_PARALLELES = AddControl(pParalleles);
	AddControl(new cgLabel(65+l,t+155," Parallele(s)"));

	pNbiter = new cgSpinner(10+l,t+175,2,0,99);
	SPIN_NBITER = AddControl(pNbiter);
	AddControl(new cgLabel(65+l,t+190," Nb iteration(s)"));
   ...
   pUtiliserRadiosite->SetValue(pFenAff3D->Display()->Settings()->UtiliserRadiosite());
...
void FenetreSetting::OnChange(CONTROL_ID ID)
{
	...
	else if ( ID == CHKBOX_RADIOSITE )
	{
		pFenAff3D->Display()->Settings()->UtiliserRadiosite(pUtiliserRadiosite->GetValue());
		pFenAff3D->Redraw();
	}
