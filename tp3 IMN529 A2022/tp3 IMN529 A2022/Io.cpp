...
#define	NB_TO	14
static	chaine	vno[NB_TO] = { "FI", "OC", "PT", "SG", "TI", "PG", "SP", "CL", "PL", "DR", "VO", "FR", "LN", "QD"};
Type	vto[NB_TO] = {unIndefini, unEnsemble, unPoint, unSegment, unTriangle, unPolygone, uneSphere, unCylindre, unPlan, uneDroite, unVolume, unFractal, unVide, unQuadrillataire};

static	entier	NbObjets = 0;
static	const	Objet	*Objets[1000];
...
void	Lire_vecteur(Fichier &f, vecteur *v)
{
	...
	if ( !f.mreel(v1) )
	{
		printf("Reel attendu V1\n");
		exit(1);
	}
	if ( !f.mreel(v2) )
	{
		printf("Reel attendu V2\n");
		exit(1);
	}
	if ( !f.mreel(v3) )
	{
		printf("Reel attendu V3\n");
		exit(1);
	}
...
void	Lire_point(Fichier &f, point *p)
{
	...
	if ( !f.mreel(v1) )
	{
		printf("Reel attendu P1\n");
		exit(1);
	}
	if ( !f.mreel(v2) )
	{
		printf("Reel attendu P2\n");
		exit(1);
	}
	if ( !f.mreel(v3) )
	{
		printf("Reel attendu P3\n");
		exit(1);
	}
...
void	Lire_Sommet(Fichier &f, point* &p)
{
	...
	if ( !f.mreel(v1) )
	{
		printf("Reel attendu So1\n");
		exit(1);
	}
	if ( !f.mreel(v2) )
	{
		printf("Reel attendu So2\n");
		exit(1);
	}
	if ( !f.mreel(v3) )
	{
		printf("Reel attendu So3\n");
		exit(1);
	}
...
Couleur Lire_Couleur(Fichier &f)
{
	...
	if ( !f.mreel(r) )
	{
		printf("Reel attendu C1\n");
		exit(1);
	}
	if ( !f.mreel(g) )
	{
		printf("Reel attendu C2\n");
		exit(1);
	}
	if ( !f.mreel(b) )
	{
		printf("Reel attendu C3\n");
		exit(1);
	}
	...
	for (;;) 
	{
		...
		switch (sorte) 
		{
			...
			case 7 : 
				...
				if ( !f.mreel(r) )
				{
					printf("Reel attendu, Attributs Textures\n");
					exit(1);
				}
...
Objet* Lire_Quadrillataire(Fichier& f){
     int nbsub1, nbsub2; // Nombre de subdivision dans les 2 sens.
	if ( !f.mentier(nbsub1))	{
		printf("Entier attendu, nombre de subdivision 1 Quadrillataire\n");
		exit(1);
	}
	if ( !f.mentier(nbsub2)){
		printf("Entier attendu, nombre de subdivision 2 Quadrillataire\n");
		exit(1);
	}
   Quadrillataire*   q = new Quadrillataire(nbsub1, nbsub2);
   point p;
   for(int i=0;i<4;i++){
   	Lire_point(f, &p);
   	q->ajoute(p);
...
void	Lire_PointVol(Fichier &f, PointVol *p)
{
	...
	if ( !f.mreel(v1) )
	{
		printf("Reel attendu, PtVol1\n");
		exit(1);
	}
	if ( !f.mreel(v2) )
	{
		printf("Reel attendu, PtVol2\n");
		exit(1);
	}
	if ( !f.mreel(v3) )
	{
		printf("Reel attendu, PtVol3\n");
		exit(1);
	}
...
Objet*	Lire_Volume (Fichier &f)
{
	...
	if ( !f.mreel(w) )
	{
		printf("Reel attendu, Vol Tension\n");
		exit(1);
	}
	...
	if ( !f.mentier(subdiv) )
	{
		printf("Entier attendu, Vol Niv Sub\n");
		exit(1);
	}
	...
	{
		printf("Entier attendu, Vol Niv Sub Mem\n");
		exit(1);
	}
	...
	if ( !f.mentier(nb_pts) )
	{
		printf("Entier attendu, Vol Nb Pts\n");
		exit(1);
	}
	...
	if ( !f.mentier(nb_tri) )
	{
		printf("Entier attendu, Vol Nb Tri\n");
		exit(1);
	}
	for (i=1; i<=nb_tri; i+=1) 
	{
		if ( !f.mentier(nopt1) )
		{
			printf("Entier attendu, Vol Tri %i Opt 1\n", i);
			exit(1);
		}
		if ( !f.mentier(nopt2) )
		{
			printf("Entier attendu, Vol Tri %i Opt 2\n", i);
			exit(1);
		}
		if ( !f.mentier(nopt3) )
		{
			printf("Entier attendu, Vol Tri %i Opt 3\n", i);
			exit(1);
		}
...
{
	//booleen	Lire_Objet2 (Fichier &, const Objet* &);
	Objet	*c;
	Attributs	a;
	...
	for (;;) 
	{
		if (Lire_Objet2 (f, (const Objet*&) c))
		{
			Lire_Attributs (f, &a);
			o->ajoute (c, a);
		} else 
...
booleen	Lire_Objet2 (Fichier &f, const Objet* &o)
{
	...
	switch (t) 
	{
		...
		case unQuadrillataire : p = Lire_Quadrillataire(f); break;
...
{
	booleen	Lire_Arbre (Fichier &, const Objet* &);
	Objet	*c;
...
booleen	Lire_Arbre (Fichier &f, const Objet* &o)
{
	...
	switch (t) 
	{
		...
		case unQuadrillataire :
		     p = Lire_Quadrillataire(f); break;
		case uneSphere : 
