...
// Affiche un triangle p1,p2,p3 avec les vecteurs normaux n1, n2, n3 ayant une tranformation "v" et des attributs non-géométrique "a"
{
	if ( pFenAff3D->Display()->Settings()->OpenGL() ) 
	{
		point	q1 = v.transforme(p1);
...
void Cylindre::Afficher(const Transformation& v, const Attributs& a0) const
{
	...
	for (m=0, a=0; m<nm; m+=1, a+=am) {
		...
		p0 = point(0.0,cosa,sina);
		p1 = point(0.0,cosa2,sina2);
		b = PI;

		p3 = point(b/PI,cosa,sina);
		p2 = point(b/PI,cosa2,sina2);
		...
		if ( pFenAff3D->Display()->Settings()->PolygoneRemplit() )
		{
			...
			Affiche_Tetragone(p0,p1,p2,p3, n0,n1,n2,n3, t, a1);
		} 
		else 
			Affiche_Tetragone(p0,p1,p2,p3, t, a1);
...
void Polygone::Afficher(const Transformation& v, const Attributs& a0) const
{
	...
	Attributs	a1 = a0 * ((Objet *)this)->attributs();
	if(radiosite && pFenAff3D->Display()->Settings()->UtiliserRadiosite())
		a1.diffus(radiosite[0]);
...
// Voir aussi le fichier fractal.h
...
// ...
	// Pour afficher un triangle : Affiche_Trigone(p1,p2,p3,n,n,n, t, a1);
	// avec "n" étant le vecteur normal.  Voir cette procédure ci-dessus.

}
void	 Quadrillataire::Afficher(const Transformation& v, const Attributs& a0) const{
	...
	vecteur v1 = sommet(2)-sommet(1);
	vecteur v2 = sommet(sommets())-sommet(1);
	point pts[4];
	Couleur couleurs[4];
	int k;

	for(int i=0;i<nombreSubdivision1;i++){
      	for(int j=0;j<nombreSubdivision2;j++){
      	   pts[0] = sommet(1) + (((reel)i + 0.0)/nombreSubdivision1)*v1 + (((reel)j + 0.0)/nombreSubdivision2)*v2;
      	   pts[1] = sommet(1) + (((reel)i + 1.0)/nombreSubdivision1)*v1 + (((reel)j + 0.0)/nombreSubdivision2)*v2;
      	   pts[2] = sommet(1) + (((reel)i + 1.0)/nombreSubdivision1)*v1 + (((reel)j + 1.0)/nombreSubdivision2)*v2;
      	   pts[3] = sommet(1) + (((reel)i + 0.0)/nombreSubdivision1)*v1 + (((reel)j + 1.0)/nombreSubdivision2)*v2;

      	   for(k=0;k<4;k++){
			point p2 = t.transforme(pts[k]);
			vx[k] = p2.x();
			vy[k] = p2.y();
			vz[k] = p2.z();
      	   }

		vecteur vv1(vx[1]-vx[0],vy[1]-vy[0],vz[1]-vz[0]);
		vecteur vv2(vx[2]-vx[1],vy[2]-vy[1],vz[2]-vz[1]);
		vecteur normal = vv1^vv2;
	   ...
	   if(radiosite && pFenAff3D->Display()->Settings()->UtiliserRadiosite())
		  a1.diffus(radiosite[j*nombreSubdivision1+i]);


       if(radiosite && pFenAff3D->Display()->Settings()->UtiliserRadiosite()){
          if(pFenAff3D->Display()->Settings()->Gouraud()){
                     couleurs[0] = MoyenneCoinPatch(0+i,0+j);
                     couleurs[1] = MoyenneCoinPatch(1+i,0+j);
                     couleurs[2] = MoyenneCoinPatch(1+i,1+j);
                     couleurs[3] = MoyenneCoinPatch(0+i,1+j);
                  pFenAff3D->Display()->Polygone(4, vx,vy,vz, nx,ny,nz, couleurs, a1);
           }
		  else{
                 a1.diffus( radiosite[i+j*nombreSubdivision1] );
                 pFenAff3D->Display()->Polygone(4, vx,vy,vz, nx,ny,nz, a1);
          }
	   }
	   else
       	  pFenAff3D->Display()->Polygone(4, vx,vy,vz, nx,ny,nz, a1); 
...
Couleur Quadrillataire::MoyenneCoinPatch(int x, int y) const{
 Couleur r(0,0,0);
 for(int a=x-1;a<=x;a++)
  for(int b=y-1;b<=y;b++){
   int i=a, j=b;
   if(i<0) i =0;
   else if(i>=nombreSubdivision1) i = nombreSubdivision1-1;
   if(j<0) j =0;
   else if(j>=nombreSubdivision2) j = nombreSubdivision2-1;
   r=r+radiosite[i+j*nombreSubdivision1];
   }
   return r*0.25;
}
