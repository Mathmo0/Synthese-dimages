...
void Display::SetupLighting(Camera* pCamera)
{
	if ( pCamera->AvecLumiere() && pCamera->LumiereActiveScene() && Settings()->PolygoneRemplit() )
	{
		...
		{
			Lumiere Lum = pCamera->GetLumiere(i);
			if ( Lum.Active() )
			{
				point p2 = Lum.Position();
				GLfloat lightPos[] = { float(p2.x()),float(p2.y()),float(p2.z()), 1};

				Couleur ambiante = Lum.Ambiante();
				float ambientLight[] = {float(ambiante.rouge()), float(ambiante.vert()), float(ambiante.bleu()), 1.0f};
				Couleur diffuse = Lum.Diffuse();
...
{
	float v[4] = { float(c.rouge()), float(c.vert()), float(c.bleu()), 1.0 };
...
void Display::Polygone(int n, double x[], double y[], double z[],
				double nx[], double ny[], double nz[],
				...
				const Attributs& a)
{
	...
	if ( Settings()->PolygoneRemplit() && !a.highlight() )
	{
		...
		{
			glNormal3f(nx[i],ny[i],nz[i]);
			glColor3f(couleurs[i].rouge(), couleurs[i].vert(), couleurs[i].bleu());
			glVertex3f(x[i], y[i], z[i]);
		}
		glEnd();
		if(light)
