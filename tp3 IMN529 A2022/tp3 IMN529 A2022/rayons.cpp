...
booleen TraceRayons(const Camera& camera, Objet *scene, const entier& resx,const entier& resy, char fichier[])
// Genere un rayon pour chacun des pixel et calcul l'intensite de chacun
{ 
	Couleur Intensite;

	entier nb_pixel_x = resx;
	entier nb_pixel_y = resy;
...
// ...
	...
	f.Wchaine("P6\r");
	f.Wentier(resx); f.Wcarac(' '); f.Wentier(resy);	f.Wchaine("\r");
