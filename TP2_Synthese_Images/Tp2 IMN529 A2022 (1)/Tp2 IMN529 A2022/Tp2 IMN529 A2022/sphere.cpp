...
void Sphere::mapUV(const point&p, reel& x, reel& y){
	vecteur v = gradient(p);
	v.normalise();
	reel t = sqrt(v.x()*v.x() + v.y() * v.y());

	if(t>0)
		x = v.y() > 0 ? acos(v.x()/t)/(2*PI) : 1-acos(v.x()/t)/(2*PI);

	y = 0.5 + asin(v.z())/PI;
}
void Sphere::getUVsensTexture(const point&p, vecteur& u, vecteur& v){
	vecteur vn = gradient(p);
	vn.normalise();
	u = vecteur(v.x(), v.y(), 0);
	u.normalise();
	v = u ^ vn;
