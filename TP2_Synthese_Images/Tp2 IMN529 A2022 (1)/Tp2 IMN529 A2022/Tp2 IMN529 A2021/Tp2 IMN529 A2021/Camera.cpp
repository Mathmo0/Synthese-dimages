...
//--------------------
void Camera::AddLumiere(const Lumiere& lum)
{
...
Lumiere& Camera::GetLumiere(entier i)
{
	assert(i >= 0);
...
const Couleur& Camera::Ambiante(const int& i) const
{
	assert(i >= 0);
	assert(i < m_nNbLumiere);

	return m_Lumiere[i].Ambiante();
}

const Couleur& Camera::Diffuse(const int& i) const
{
	assert(i >= 0);
	assert(i < m_nNbLumiere);

	return m_Lumiere[i].Diffuse();
}

void Camera::Ambiante(const int& i, Couleur c)
{
	assert(i >= 0);
	assert(i < m_nNbLumiere);

	m_Lumiere[i].Ambiante(c);
}

void Camera::Diffuse(const int& i, Couleur c)
{
	assert(i >= 0);
	assert(i < m_nNbLumiere);

	m_Lumiere[i].Diffuse(c);
}
