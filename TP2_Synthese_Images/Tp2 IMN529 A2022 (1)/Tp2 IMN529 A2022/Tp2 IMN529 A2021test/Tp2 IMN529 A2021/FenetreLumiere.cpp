...
void FenetreLumiere::SetAmbiante()
{
	...
	pFenAff3D->Scene()->GetpCamera()->Ambiante(pChoixLumiere->GetValue()-1, Couleur(valR, valG, valB) );
	pFenAff3D->Redraw();
}
...
void FenetreLumiere::SetDiffuse()
{
	...
	pFenAff3D->Scene()->GetpCamera()->Diffuse(pChoixLumiere->GetValue()-1, Couleur(valR, valG, valB) );
	pFenAff3D->Redraw();
}
...
void FenetreLumiere::ChangeLumiere()
{
	...
	Couleur ambiante = pFenAff3D->Scene()->GetpCamera()->Ambiante(pChoixLumiere->GetValue()-1);
	pLumAmbR->SetValue(ambiante.rouge());
	pLumAmbG->SetValue(ambiante.vert());
	pLumAmbB->SetValue(ambiante.bleu());

	Couleur diffuse = pFenAff3D->Scene()->GetpCamera()->Diffuse(pChoixLumiere->GetValue()-1);
